...
#put this in a table
#maybe add logistic regression for HTN
model <- lm(MAP ~ RIDRETH1 + Year + RIDAGEYR + RIAGENDR, data = modelData)
summary(model)
```

```{r corr_matrix}
#something broke with this, but i dont really think we need this plot tbh
library(corrplot)
...
```{r year_boxplot}
modelData %>% ggplot(aes(x = Year, y = MAP)) +
  geom_boxplot() +
...
```{r gender_boxplot}
modelData %>% ggplot(aes(x = RIAGENDR, y = MAP)) +
  geom_boxplot() +
  stat_smooth(method = "lm", se = FALSE) +
...
```{r ethnicity_boxplot}
modelData %>% ggplot(aes(x = RIDRETH1, y = MAP)) +
  geom_boxplot() +
  stat_smooth(method = "lm", se = FALSE) +
