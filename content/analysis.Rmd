...
Our team wanted to cast a wide net to cover many factors and relationships so that we can pinpoint significant factors. This way, we can provide evidence for our thesis to show that race/ethnicity and time greatly impact general health trends. As such, we aimed to investigate different predictors of health in relation to MAP. We hope to better illustrate our initial exploration of time and race/ethnicity with MAP by profiling other variables.
...
Since we are primarily looking at chronological data of MAP in relation to multiple indicators of health, we decided on choosing linear regression as our primary modeling method. FINISH ANALYSIS TO COMPLETE THIS SECTION.
...
```{r lin_reg_model_summary, echo=FALSE}
...
modelData %>% lm(formula = MAP ~ factor(RIDRETH1) + factor(Year) + RIDAGEYR + factor(RIAGENDR)) %>% 
  broom::tidy() %>%
  knitr::kable(format = "markdown")
```

```{r, echo=FALSE}
model_interaction <- lm(MAP ~ factor(RIDRETH1) + factor(Year) + RIDAGEYR + factor(RIAGENDR) +  
                        factor(RIDRETH1):factor(RIAGENDR), data = modelData)
...
modelData %>%
  filter(Year == "2018") %>%
  ggplot(aes(RIDAGEYR, MAP)) + geom_point(aes(color = RIAGENDR)) 
```
...
year_facet <- age_plot + facet_wrap( ~ Year, ncol=4)+
  labs(title = "Mean Arterial Pressure (MAP) vs. Age by Year", 
...
ethnicity_facet <- age_plot + facet_wrap( ~ RIDRETH1)+
  labs(title = "Mean Arterial Pressure (MAP) vs. Age by Ethnicity", 
       x = "Age", y = "Mean Arterial Pressure")
ethnicity_facet
```
...
gender_facet <- age_plot + facet_wrap( ~ RIAGENDR)+
  labs(title = "Mean Arterial Pressure (MAP) vs. Age by Gender", 
       x = "Age", y = "Mean Arterial Pressure")
gender_facet
```
...
FINISH ANALYSIS TO COMPLETE THIS SECTION.

## Clarity of Figures and Explanations:
Include significant relationships of MAP with impactful variables
Describe why each relationship is significant (Maybe why cholesterol is important health indicator, or why age is a good benchmark for MAP)
Provide a small blurb/explanation of the relationship being shown/plotted
Maybe some further interpretation of the relationship if it’s significant
